...
import { Search, Tag, Save, AlertCircle, Loader2, Image as ImageIcon, Store } from 'lucide-react';
...
export default function BranchDiscountsPage() {
  ...
  const fetchBranchesAndProducts = async () => {
    ...
    try {
      const [branchesRes, productsRes] = await Promise.all([
        ...
        supabase.from('products').select('id, sku, name, price, image_url').eq('is_active', true).order('name')
      ]);
  ...
  const handleSelectBranch = async (branchId: number) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('id, product_id, discount_price, discount_end_date, status')
  ...
  const handleSave = async () => {
    ...
    try {
      const modifiedProductIds = Array.from(new Set([...Object.keys(edits), ...Object.keys(editsDate)]));
      const updates = modifiedProductIds.map((productId) => {
        ...
        if (inv) {
          ...
          return supabase
            .from('branch_inventory')
            .update({ discount_price: discountPrice, discount_end_date: discountEndDate })
            .eq('id', inv.id);
        } else {
      ...
      toast.success('บันทึกราคาโปรโมชั่นสำเร็จ');
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
            ...
            ตั้งค่าราคาลดพิเศษ (Discount)
          </h1>
          <p className="text-sm text-gray-500 mt-1">กำหนดราคาโปรโมชั่นแยกตามรายสาขา (Super Admin)</p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving || (Object.keys(edits).length === 0 && Object.keys(editsDate).length === 0)}
        ...
        >
          ...
          บันทึกการเปลี่ยนแปลง {Object.keys(edits).length > 0 && `(${Object.keys(edits).length})`}
        </button>
      </div>
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 flex flex-col md:flex-row gap-4">
        <div className="w-full md:w-1/3 relative">
          ...
          <select
            ...
            title="เลือกสาขาที่ต้องการตั้งค่าราคา"
                  ...
                  return (
                    <tr key={product.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition">
                      ...
                      <td className="px-6 py-4">
                        ...
                        {inv?.status === 0 && (
                          <span className="inline-block mt-1 px-2 py-0.5 bg-gray-100 text-gray-500 text-[10px] rounded-full">สาขานี้ปิดการขาย</span>
                        )}
                      </td>
